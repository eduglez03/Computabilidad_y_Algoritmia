...
// sobrecarga del operador union
Lenguaje operator+(const Lenguaje& kLenguaje1, const Lenguaje& kLenguaje2) {
  return kLenguaje1.Concatenacion(kLenguaje2);
} 

// sobrecarga del operador diferencia
Lenguaje operator-(const Lenguaje& kLenguaje1, const Lenguaje& kLenguaje2) {
  return kLenguaje1.Diferencia(kLenguaje2);
}

// sobrecarga del operador de intersección
Lenguaje operator^(const Lenguaje& kLenguaje1, const Lenguaje& kLenguaje2) {
  return kLenguaje1.Interseccion(kLenguaje2);
}  

// sobrecarga del operador de union
Lenguaje operator|(const Lenguaje& kLenguaje1, const Lenguaje& kLenguaje2) {
  return kLenguaje1.Union(kLenguaje2);
} 

// sobrecarga del operador de contrario para hacer la inversa
Lenguaje operator!(const Lenguaje& kLenguaje1) {
  return kLenguaje1.Inversa();
}

// sobrecarga del operador de multiplicación para hacerlo potencia
Lenguaje operator*(const Lenguaje& kLenguaje, const int kPotencia) {
  return kLenguaje.Potencia(kPotencia);
}  


//------------------Ejercicios-Practica------------------//

// Quita las cadenas vacias de sobra que s eproduce al concatenar varias de estas
Cadena QuitaCadenasVaciasSobra(const Cadena& cadena_comprobar) {
  Cadena cadena_final;
  if (cadena_comprobar.Longitud() == 1 && cadena_comprobar.AtCadena(0).GetSimbolo() == "&") {
    cadena_final = cadena_comprobar;
  } else {
    for (int bucle{0}; bucle < cadena_comprobar.Longitud(); ++bucle) {
      if (cadena_comprobar.AtCadena(bucle).GetSimbolo() != "&") {
        cadena_final.ConcatenarDelante(cadena_comprobar.AtCadena(bucle).GetSimbolo());
      }
    }
  }
  return cadena_final;
...
Lenguaje Lenguaje::Concatenacion(const Lenguaje& kLenguajeOperar) const {
  ...
  for (auto bucle1 : lenguaje_) {  // recorremos el primer conjunto buscando las primeras partes de la cadena
    for (auto bucle2 : kLenguajeOperar.GetLenguaje()) {    // recorremos el segundo conjunto formando las nuevas cadenas
      ...
      cadena_anadir.SetCadena(QuitaCadenasVaciasSobra(cadena_anadir));
...
// metodo que devuelve un lenguaje a partir de subcadenas
Lenguaje Lenguaje::LenguajeSubcadenas() const {
  Lenguaje lenguaje_final;
  for (auto bucle1 : lenguaje_) {
    std::vector<Cadena> vector_cadena{bucle1.Subcadenas()};
    for (int bucle2{0}; bucle2 < vector_cadena.size(); ++bucle2) {
      lenguaje_final.AnadirLenguaje(vector_cadena[bucle2]);
    }
  }
  return lenguaje_final;
}

// metodo que crea un alfabeto que contiene todas los simbolos del lenguaje
Alfabeto Lenguaje::CreaAlfabetoDeLenguaje() const {
  Simbolo simbolo_vacio{""};
  Alfabeto alfabeto_final{simbolo_vacio};
  for (auto bucle1 : lenguaje_) {
    for (int bucle2{0}; bucle2 < bucle1.Longitud(); ++bucle2) {
      if (bucle1.AtCadena(bucle2).GetSimbolo() != "&") {   // si es la cadena vacía no hay que meterla en el alfabeto
        alfabeto_final.AnadirAlfabeto(bucle1.AtCadena(bucle2));
      }
    }
  }
